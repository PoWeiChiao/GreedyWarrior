...
class DipFinder:
    def __init__(self, vix_threshold: float = 30.0, vix_rsi2_threshold: float = 90.0, sp500_rsi2_threshold: float = 30.0, sp500_sma200: bool = True):
    ...
    def find_dips(self, start_date: str, end_date: str) -> list:
        ...
        for date in vix:
            ...
            if vix[date].high >= self.vix_threshold and vix[date].rsi2 >= self.vix_rsi2_threshold and sp500[date].rsi2 <= self.sp500_rsi2_threshold:
                dips.append(date)
                print(f"[{date}] (VIX: {vix[date].high:.2f}) (VIX RSI2: {vix[date].rsi2:.2f}) (SP500 RSI2: {sp500[date].rsi2:.2f}) (SP500: {sp500[date].close:.2f}) (SP500 SMA50: {sp500[date].sma_50:.2f}) (SP500 SMA200: {sp500[date].sma_200:.2f})")
        return dips